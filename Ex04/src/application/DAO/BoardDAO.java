...
public class BoardDAO extends JDBConnection {
	...
	public List<Board> list() {
		// 게시글 목록을 담을 컬렉션(ArrayList) 객체 생성
		List<Board> boardList = new ArrayList<Board>();
		
		// SQL 작성
		// 자바에서 sql코드를 써넣을 때는 뒤에 세미콜론; 은 빼고 쓴다.
		String sql = " SELECT * "
				   + " FROM board ";
		
		// 자바에서 외부에 있는 데이터에 접근할 때에는 try-catch 사용하는게 좋다.
		try {
	...
	public int update(Board board) {
		System.out.println("수정 요청된 게시물: " + board);
		int result = 0;			// 결과 : 적용된 데이터 개수
		...
		try {
			...
			result = psmt.executeUpdate();				// SQL 실행 요청
			 System.out.println("실제 DB 반영된 행 수: " + result);
			// * executeUpdate() 
			// SQL(INSERT, UPDATE, DELETE) 실행 시 적용된 데이터 개수를 int 타입으로 받아온다.
			// ex) 게시글 1개 적용 성공 시, result : 1 
			//				    실패 시, result : 0
	...
	public int views(int no) {
		...
		try {
			...
			System.err.println("view 조회 시, 예외 발생");
			e.printStackTrace();
		}
		return result;
	}
